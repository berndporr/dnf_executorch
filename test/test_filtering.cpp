...
void doTest(const char* filename, float failAbove)
{
    double mu = 0.1;
    ...
    for (int i = 0; i < 500; i++)
    {
        ...
        if ((i > 450) && (fabs(output_signal) > failAbove))
        {
...
int main(int, char **)
{
    doTest("dnf_executorch1.pte",0.0005);
    doTest("dnf_executorch5.pte",0.0005);
}
